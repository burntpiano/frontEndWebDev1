...
let myOtherRandom = Math.floor(Math.random(1) * 10);

let myLength = myValue.length;

let myCase = String("aLpHaBeT").toLocaleLowerCase();

let myName = "Rory";

// let myNameLetter = myName.length.charAt.(Math.floor(Math.random(1) * 4));

let myNameLetter = myName.charAt(Math.floor(Math.random(0), myName.length));
